...
class AzulSESupabaseUser extends BaseAuthUser {
  AzulSESupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> azulSESupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AzulSESupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
